...
The demo application uses the *fabric8 maven plugin* as an access point to Kubernetes: the application can be deployed on a WebSphere 
Liberty-based docker container running inside a Kubernetes cluster and the integration tests run dependent on the containerized 
application.
...
3. If you use *[minikube]*, the cluster must be started. To start the cluster use the following command:
...
(if you use *VirtualBox* to run the *minikube* virtual machine, `-DtestProxyHost=192.168.99.100` helps you specify the virtual 
machine host.)
